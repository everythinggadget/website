<?php
// cart.php
?>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Cart - Game Arena</title>
  <link href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css" rel="stylesheet">
</head>
<body class="bg-gray-100 text-gray-900">
  <header class="bg-white shadow">
    <div class="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
      <h1 class="text-2xl font-bold">Game Arena</h1>
      <nav class="space-x-4">
        <a href="index.php" class="hover:text-blue-600">Home</a>
        <a href="/p/product.php" class="hover:text-blue-600">Products</a>
        <a href="cart.php" class="hover:text-blue-600 font-semibold">Cart</a>
        <a href="contact.php" class="hover:text-blue-600">Contact</a>
      </nav>
    </div>
  </header>

  <main class="max-w-3xl mx-auto px-4 py-16 text-center">
    <h2 class="text-3xl font-bold mb-6">Your Cart</h2>
    <p class="text-gray-600 mb-8">Ready to place your order? Message us directly on WhatsApp.</p>
    <a href="https://wa.me/2348158236154" target="_blank" class="inline-block bg-green-600 text-white text-lg font-semibold px-6 py-3 rounded hover:bg-green-700 transition">Message on WhatsApp</a>
  </main>

  <footer class="bg-white shadow mt-12">
    <div class="max-w-7xl mx-auto px-4 py-6 text-center text-gray-500">
      &copy; <?php echo date('Y'); ?> Game Arena. All rights reserved.
    </div>
  </footer>
</body>
</html>
